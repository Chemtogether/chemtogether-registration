...
from ..models import Form, AbstractForm, FormEntry


logger = logging.getLogger("ct_registration.forms_builder.templatetag")
...
class BuiltFormNode(template.Node):
    ...
    def render(self, context):
        ...
        try:
            instance = FormEntry.objects.filter(author=user.company.get()).filter(form=form).order_by("-entry_time")[0]
        except:
            instance = None
